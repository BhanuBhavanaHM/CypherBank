<?php
    ...
    $amnt = mysqli_real_escape_string($conn, $_POST["amt"]);
    ...
    $sql1 = "SELECT balance FROM passbook".$_SESSION['cust_id']." ORDER BY trans_id DESC LIMIT 1";
    $result1 = $conn->query($sql1);
    $row = $result1->fetch_assoc();
    $DebBal = $row["balance"];

     $final_balance = $DebBal + $amnt;

    $sql2 = "INSERT INTO passbook".$_SESSION['cust_id']." VALUES(
                            NULL,
                            NOW(),
                            'Credit by Bank',
                            '0',
...
<body>
    <div class="flex-container">
        <div class="flex-item">
            <?php
                if ( ( ($conn->query($sql0)) && ($conn->query($sql2)) === TRUE) ) { ?>
